...
new_content_taiko = """const { ethers } = require('ethers');
...
new_content_taikorun = """const { ethers } = require('ethers');
const chains = require('./chains');
const provider = chains.mainnet.taiko.provider;
const changeProvider = chains.mainnet.taiko.changeRpcProvider;
const explorer = chains.mainnet.taiko.explorer;
...
def update_file(file_path, new_content):
    try:
        # Menulis ulang file dengan konten baru
...
for parent_folder in ["notrun", "firstrun"]:
    for folder_number in range(1, 39):  # Folder 1 sampai 38
        ...
        # Menentukan file target berdasarkan parent folder
        if parent_folder == "notrun":
            file_name = "taiko.js"
            new_content = new_content_taiko
        else:  # parent_folder == "firstrun"
            file_name = "taikorun.js"
            new_content = new_content_taikorun

        file_path = os.path.join(folder_path, file_name)

        # Periksa apakah file ada, lalu update
